...
export const Sales: React.FC = () => {
  ...
  const yesterdayCount = sales.filter((s) => s.sale_date && s.sale_date.startsWith(yesterdayStr)).length;
  const thisMonthCount = sales.filter((s) => s.sale_date && s.sale_date.startsWith(currentMonthStr)).length;
  const last30DaysCount = sales.filter((s) => s.sale_date && s.sale_date >= thirtyDaysAgoStr && s.sale_date <= todayStr).length;
  ...
  const filteredSales = sales
    .filter((s) => {
      ...
      if (periodFilter === 'YESTERDAY' && (!s.sale_date || !s.sale_date.startsWith(yesterdayStr))) return false;
      if (periodFilter === 'THIS_MONTH' && (!s.sale_date || !s.sale_date.startsWith(currentMonthStr))) return false;
      if (periodFilter === 'LAST_30_DAYS' && (!s.sale_date || s.sale_date < thirtyDaysAgoStr || s.sale_date > todayStr)) return false;
      return true;
    })
    .sort((a, b) => {
  ...
  const handleExportCSV = () => {
    const headers = ['Sale Number', 'Date', 'Customer', 'Items Details', 'Total Amount', 'Payment Status'];
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1 text-xs pt-2 border-t border-zinc-200 dark:border-zinc-800">
              <div className="flex justify-between font-extrabold text-sm">
                ...
                <span className="tabular-nums font-sans">{formatCurrency(selectedSale.total_amount)}</span>
              </div>
              <div className="flex justify-between text-emerald-600 dark:text-emerald-400 font-bold">
                <span>Payment Status:</span>
                <span>Paid in Full (पूर्ण भुगतान प्राप्त)</span>
              </div>
            </div>
